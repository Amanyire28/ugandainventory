...
<div id="transferModal" class="fixed inset-0 z-[70] hidden overflow-y-auto bg-gray-900/60 backdrop-blur-md flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-6 sm:p-8 relative modal-float border border-indigo-100 max-h-[85vh] overflow-y-auto">
        <button onclick="closeTransferModal()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600">
        ...
        <div class="text-center mb-6">
            ...
            <h2 class="text-2xl font-black text-gray-900">Initiate Inter-Branch Stock Transfer</h2>
            <p class="text-xs text-gray-500 mt-1 font-medium">Select source and destination branches, then add products to transfer.</p>
        </div>
        ...
        <form action="{{ route('stock-transfers.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="from_location_id" required class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                ...
                <div>
                    ...
                    <select name="to_location_id" required class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none">
            ...
            <div class="border-t border-b border-gray-100 py-4 my-2">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <button type="button" onclick="addTransferRow()" class="text-xs font-black text-indigo-600 hover:text-indigo-800 flex items-center">
                ...
                <div id="transferRows" class="space-y-3">
                    <div class="flex items-center space-x-3 transfer-row">
                        <div class="flex-1">
                            <select name="products[0][id]" required class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        ...
                        <div class="w-32">
                            <input type="number" step="0.01" min="0.01" name="products[0][qty]" required placeholder="Qty" class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="2" class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Reason or details for stock transfer..."></textarea>
            </div>

            <div class="pt-4 flex items-center justify-end space-x-3">
                <button type="button" onclick="closeTransferModal()" class="px-5 py-2.5 bg-gray-100 text-gray-700 font-bold rounded-xl text-xs hover:bg-gray-200">Cancel</button>
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl text-xs shadow-md">Complete Transfer</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function addTransferRow() {
        ...
        row.innerHTML = `
            <div class="flex-1">
                <select name="products[${transferRowIdx}][id]" required class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                    ${firstSelectHTML}
                </select>
            </div>
            <div class="w-32">
                <input type="number" step="0.01" min="0.01" name="products[${transferRowIdx}][qty]" required placeholder="Qty" class="w-full px-3.5 py-2 border border-gray-300 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-500 focus:outline-none">
            </div>
